let selected_files_overview = new Set();
...
$('#example-multiple-optgroups').multiselect({
    ...
    onChange: function (option, checked) {
        ...
        if (checked) {
            selected_files_overview.add(current_selected_file_name)
        } else {
            selected_files_overview.delete(current_selected_file_name)
        }
    },
    onDropdownHidden: function (event) {
        if (selected_files_overview.size > 0) {
            layer.load(0, {shade: false});
            let selectedFilesString = Array.from(selected_files_overview).join(',');
            let encodedSelectedFiles = encodeURIComponent(selectedFilesString);
            window.location.href = '/temperature/overview?fileId=' + encodedSelectedFiles;
        }
    }
});
